...
showFullName.apply(user, ['Pizzy nie ma, ']) // 'Marek Jazur';
```
---
```
function showFullName(phrase) {
...
var getMarek = showFullName.bind(user); // function with this -> user;

getMarek('Pizzy nie ma, ')  // 'Pizzy nie ma, Marek Jazur';
```
---
```
function showFullName(phrase) {
...
var getMarek = showFullName.bind(user, 'Pizza jest, '); // function with this -> user, and defined first argument;

getMarek('Pizzy nie ma, ')  // 'Pizza jest, Marek Jazur';
```
---
function login() {
  // magic is here;
